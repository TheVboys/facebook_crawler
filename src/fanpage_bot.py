...
=======
        ...
        service = Service(executable_path=driver_location)
        # options.binary_location = binary_location # remove this on windown
>>>>>>> e0519f8c4d42fda4378a0b035f73d604bdd2b655
        ...
        self.browser = Chrome(service=Service(ChromeDriverManager().install()), options=options)
    ...
    def get_all_post_from_fanpage(self, start=105):
    ...
    def get_reactors(self, start=1272):
        post_links = pd.read_csv("./datasets/post_fanpage.csv")[start:]
        ...
        for index, post in post_links.iterrows():
            ...
            if post_id is not None:
                for reactors in get_reactors(post_id, cookies='./cookies/facebook_cookies.txt',
                                    options={"allow_extra_requests": True,
                                            "reactions": True, 'reactors': True},
                                    ):
                    links.append(reactors["link"])
                    user_names.append(reactors["name"])
                self.save_to_csv(links, fanpage_name, "./datasets/reactors_v2.csv", "user_url")
        try:
            self.browser.quit()
        except:
            pass
